    ...
    >
      <div className="space-y-[1px] bg-[var(--border-strong)]">
        {rows.length ? rows.map((item, index) => (
          <div key={`${item.type}-${item.description || item.id || index}`} className="grid grid-cols-1 md:grid-cols-[170px_1fr] gap-[1px] bg-[var(--border-strong)]">
            ...
            <div className="bg-[var(--surface-panel)] p-3">
              ...
              {(item.asset_id || item.raw_tag || item.decision_id) && (
                <p className="caption-mono text-[var(--data-mono)] mt-1 machine-token">{[item.asset_id, item.raw_tag, item.decision_id].filter(Boolean).join(' / ')}</p>
              )}
            </div>
          </div>
