        ...
        {brief && !loading && (
          <div className="industrial-panel-subtle p-4">
            <div className="flex items-center gap-3 mb-4 pb-3 border-b border-[var(--border-strong)]">
              ...
              {brief.source === 'claude' && (
                <span className="caption-mono status-warning" title="This brief was generated by an AI language model. Verify all readings against physical instruments before acting.">
                  AI-generated — verify against field
                </span>
              )}
            </div>

            {/* Validation warnings: ungrounded sensor tags in AI brief */}
            {!!brief.validation?.ungrounded_tags?.length && (
              <div className="mb-4 border border-[var(--warning,#f4a100)] bg-[var(--surface-base)] p-3">
                <p className="label-caps status-warning mb-1">
                  Unverified tags in brief ({brief.validation.ungrounded_tags.length})
                </p>
                <p className="caption-mono text-[var(--text-muted)] mb-2">
                  The following sensor IDs appear in the brief but were not found in structured system data.
                  Do not act on these references without independent confirmation.
                </p>
                <div className="flex flex-wrap gap-2">
                  {brief.validation.ungrounded_tags.map((tag) => (
                    <span key={tag} className="industrial-badge status-warning">{tag}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {brief.generation_error && (
              <div className="mb-3 caption-mono status-warning">
                Note: AI generation failed ({brief.generation_error}). Showing deterministic fallback.
              </div>
            )}
