...
def _derive_trust_states(confidence: list[dict], readings: list[dict], mass_balance: dict | None, model_key: str | None = None) -> list[dict]:
    ...
    for item in confidence:
        ...
        if not reading_available:
            ...
            trust_reason = "No current live provider sample is available for this tag."
            decision_basis_allowed = False
...
async def generate_compliance_report(
    ...
    db: Session = Depends(get_db),
    user: dict = Depends(require_role("Manager", "Auditor")),
):
    ...
    generated_by = user.get("username") or "unknown"
    generated_role = user.get("role") or "unknown"
    ...
    try:
        ...
        report["report_event_id"] = report_event.get("event_id")
    except Exception:
        report["operational_event_id"] = None
        report["report_event_id"] = None
...
def _format_compliance_report_text(report: dict) -> str:
    ...
    # ### = document title, ## = section heading, - = bullet (parsed by _build_simple_pdf)
    lines = [
        "### ConfidenceOS Operational Summary Report",
        f"Plant: {report.get('plant_name')} ({report.get('plant_id')})  |  Type: {report.get('report_type')}  |  Period: {report.get('period_hours')}h",
        f"Generated: {report.get('generated_at')}",
        f"By: {report.get('generated_by', 'unknown')} ({report.get('generated_role', 'unknown')})",
        f"Event ID: {report.get('report_event_id') or report.get('operational_event_id') or 'pending ledger write'}",
        "",
    ...
    lines.extend([
        ...
        f"Anomaly rows: {coverage.get('anomaly_rows', 0)}  |  Mass-balance rows: {coverage.get('mass_balance_rows', 0)}  |  Handover rows: {coverage.get('handover_rows', 0)}",
        ...
        f"Active incidents: {runtime_state.get('active_incident_count', 'not included')}  |  Active tasks: {runtime_state.get('active_verification_tasks', 'not included')}",
        f"Handover acceptance: {runtime_state.get('handover_acceptance', 'not included')}",
        f"Latest tick: {runtime_state.get('latest_tick_available', 'not included')}",
        f"Read-only boundary: {runtime_state.get('read_only_boundary', 'not included')}",
    ])
    lines.extend([
        "",
        "## Alarm Summary",
        f"Total alarms: {alarm.get('total_alarms', 0)}  |  Rate/hour: {alarm.get('alarm_rate_per_hour', 0)}",
        "By severity:",
    ])
    if severity_items:
    ...
    if reliability:
        for sid, rel in sorted(reliability.items()):
            ...
            lines.append(
                ...
                f"cal age {rel.get('calibration_age', 'n/a')}d, "
    ...
    lines.extend([
        ...
        f"Total: {verification.get('count', 0)}  |  Active: {verification.get('active_count', 0)}  |  Handover-required: {verification.get('handover_required_count', 0)}",
    ...
    lines.extend([
        ...
        f"Status: {assumption_governance.get('status', 'not included')}  |  Approved: {summary.get('approved', 0)}  |  Due soon: {summary.get('due_soon', 0)}  |  Stale: {summary.get('stale', 0)}  |  Unapproved: {summary.get('unapproved', 0)}",
    ...
    lines.extend([
        ...
        f"Ledger rows: {operational_ledger.get('count', 0)}  |  Incidents: {ledger_summary.get('incident_events', 0)}  |  Verification: {ledger_summary.get('verification_events', 0)}  |  Handover: {ledger_summary.get('handover_events', 0)}",
    ...
    lines.extend([
        "",
        "## Provenance (Unsigned)",
        f"SHA-256: {provenance.get('content_sha256', 'n/a')}",
        f"Generator: {provenance.get('generator', 'ConfidenceOS Advisory Engine')}",
        f"By: {provenance.get('generated_by', report.get('generated_by', 'unknown'))} ({provenance.get('generated_role', report.get('generated_role', 'unknown'))})",
...
def _build_simple_pdf(text: str) -> bytes:
    ...
    def _wrap(line: str, size: int) -> list:
        mc = _max_chars(size)
        result = []
        while len(line) > mc:
            sp = line[:mc].rfind(" ")
            ...
            line = "  " + line[sp:].lstrip()
        result.append(line)
        return [ln for ln in result if ln.strip()]

    # Parse line tokens
    tokens = []
    report_title = "ConfidenceOS Report"
    generated_at = ""
    for raw in text.splitlines():
        if raw.startswith("### "):
            t = raw[4:]
            report_title = t[:55]
            tokens.append(("title", t))
        elif raw.startswith("## "):
            tokens.append(("h1", raw[3:]))
        elif raw.startswith("- "):
            tokens.append(("bullet", raw[2:]))
        elif raw.strip() == "":
            tokens.append(("spacer", ""))
        else:
            if raw.startswith("Generated:"):
                generated_at = raw.replace("Generated:", "").strip()[:16]
            tokens.append(("body", raw))
    ...
    n_total = len(pages)
    hdr_label = _pdf_escape(report_title[:60])
    gen_label = _pdf_escape(generated_at)
    for pn, page_ops in enumerate(pages, 1):
        footer = _pdf_escape(
    ...
    res = f"/Font << /F1 {font_f1} 0 R /F2 {font_f2} 0 R >>"
    ...
    for i, page_ops in enumerate(pages):
        ...
        objects.append(
            ...
            f"/Resources << {res} >> /Contents {ct} 0 R >> endobj\n".encode("ascii")
        )
        objects.append(
    ...
    objects.append(
        f"{font_f1} 0 obj << /Type /Font /Subtype /Type1 /BaseFont /Helvetica >> endobj\n".encode("ascii")
    )
    objects.append(
        f"{font_f2} 0 obj << /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >> endobj\n".encode("ascii")
    )
